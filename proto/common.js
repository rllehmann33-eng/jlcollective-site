...
export const J_PATH = 'M76.84 83.65 H107.75 V84.99 H97.09 V140.66 C97.09 156.66 92.09 158.66 82.09 158.66 C72.49 158.66 65.49 155.06 65.49 150.30 H67.49 C67.49 155.30 80.09 153.66 82.09 153.66 C88.09 153.66 87.49 152.26 87.49 140.66 V84.99 H76.84 Z';
export const L_PATH = 'M116.00 150.66H126.66V84.99H116.00V83.65H146.91V84.99H136.26V150.66H153.34Q161.79 150.66 166.59 145.38Q171.39 140.10 173.60 130.98H174.66V152.00H116.00Z';
export const RING_R = 90 / 120;        // 0.75 in unit space (240 box -> [-1,1])
export const RING_W = 6 / 120;         // 0.05
export const TICK_W = 3 / 120;
export const TICK_R0 = 84 / 120, TICK_R1 = 96 / 120;   // y 36..24 from the top edge
export const TICK_ANGLES = [Math.PI / 2, -Math.PI / 6, Math.PI + Math.PI / 6]; // 0°, 120°, 240° clockwise from top

export const reduced = matchMedia('(prefers-reduced-motion: reduce)').matches;
export const canHover = matchMedia('(hover: hover)').matches;
export const isNarrow = () => innerWidth < 720;
/** On phones the object lives in a band under the copy: returns its centre as a fraction of hero height. */
export const bandCentreY = (w, h) => 1 - (0.33 * w + 28) / h;
...
export function ringShape(inner = RING_R - RING_W / 2, outer = RING_R + RING_W / 2) {
...
export function tickShapes(w = TICK_W, r0 = TICK_R0, r1 = TICK_R1) {
 ...
 */
export function buildSeal({ depth = 0.06, bevel = 0.012, letterDepth = 0.06, letterBevel = 0.006, matBrass, matCream, curveSegments = 96 } = {}) {
...
export function setup({ antialias = true, maxDpr = 2 } = {}) {
...
export function worldAtNDC(camera, ndcX, ndcY, depthPoint, out = new THREE.Vector3()) {
...
export function pxPerUnit(camera, depthPoint, heightPx) {
